...
const AUTO_DISCOVERY_MAX_DELTA = 0.18;
const cleanMapStyle = [
...
export function HomeScreen({ session, onSignOut }: { session: { token: string; user: User }; onSignOut: () => Promise<void> }) {
  ...
  const canShowMapPins = Math.max(region.latitudeDelta, region.longitudeDelta) <= AUTO_DISCOVERY_MAX_DELTA;
  ...
  const loadSpots = useCallback(async (nextRegion: Region, mode: MapMode) => {
    ...
    try {
      ...
      setSpots((current) => current.length === nextSpots.length && current.every((spot, index) => spot.id === nextSpots[index]?.id) ? current : nextSpots);
  ...
  const handleMapMovement = useCallback((nextRegion: Region) => {
    if (Math.max(nextRegion.latitudeDelta, nextRegion.longitudeDelta) > AUTO_DISCOVERY_MAX_DELTA) return;
  ...
  return <View className="flex-1 bg-white">
    ...
    <MapView ref={mapRef} initialRegion={region} style={StyleSheet.absoluteFillObject} showsUserLocation showsMyLocationButton={false} showsPointsOfInterest={false} customMapStyle={cleanMapStyle} moveOnMarkerPress={false} toolbarEnabled={false} minZoomLevel={14} maxZoomLevel={18} onRegionChange={handleMapMovement} onRegionChangeComplete={handleMapRegionChange} onUserLocationChange={handleUserLocationChange}>
