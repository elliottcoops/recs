...
async function notify(env, recipientId, actorId, type, entityId, payload = {}) {
  if (!recipientId || recipientId === actorId) return;
  await env.DB.prepare("INSERT INTO activity (id,recipient_id,actor_id,type,entity_id,payload,created_at) VALUES (?,?,?,?,?,?,?)")
    .bind(id(), recipientId, actorId ?? null, type, entityId ?? null, JSON.stringify(payload), now())
    .run();
}
...
async function friendIds(env, userId) {
  const [rows, blocks] = await Promise.all([
    all(env.DB.prepare("SELECT requester_id, recipient_id FROM friendships WHERE status='accepted' AND (requester_id=? OR recipient_id=?)").bind(userId, userId)),
    all(env.DB.prepare("SELECT blocker_id, blocked_id FROM user_blocks WHERE blocker_id=? OR blocked_id=?").bind(userId, userId)),
  ]);
  const blockedIds = new Set(blocks.map((block) => block.blocker_id === userId ? block.blocked_id : block.blocker_id));
  return new Set(rows.map((row) => row.requester_id === userId ? row.recipient_id : row.requester_id).filter((friendId) => !blockedIds.has(friendId)));
}
async function canView(env, spot, userId) { return spot.user_id === userId || spot.visibility === "public" || (spot.visibility === "friends" && (await friendIds(env, userId)).has(spot.user_id)); }
const isWithinMiles = (spot, latitude, longitude, miles) => {
  if (![spot.latitude, spot.longitude, latitude, longitude, miles].every(Number.isFinite)) return true;
  const radians = (value) => value * Math.PI / 180;
  const deltaLatitude = radians(Number(spot.latitude) - latitude), deltaLongitude = radians(Number(spot.longitude) - longitude);
  const a = Math.sin(deltaLatitude / 2) ** 2 + Math.cos(radians(latitude)) * Math.cos(radians(Number(spot.latitude))) * Math.sin(deltaLongitude / 2) ** 2;
...
async function googleSearch(env, query, latitude, longitude, radiusMiles = 0) {
  ...
  return places.filter((place) => place.location).map((place) => ({ placeId: place.id, name: place.displayName?.text ?? "Unnamed venue", address: place.formattedAddress ?? "Address unavailable", latitude: place.location.latitude, longitude: place.location.longitude, rating: place.rating ?? 0 })).filter((place) => !radiusMiles || isWithinMiles(place, Number(latitude), Number(longitude), radiusMiles));
}
...
export default {
  async fetch(request, env) {
    ...
    try {
      ...
      if (request.method === "POST" && path === "/api/places/search") { const { query, latitude, longitude, radiusMiles } = await body(request); if (String(query ?? "").trim().length < 3) return json({ error: "Enter at least three characters." },400); const localRadius=Math.min(Math.max(Number(radiusMiles)||0,0),25); return json(await googleSearch(env, query.trim(), latitude, longitude, localRadius)); }
      ...
      if (request.method === "GET" && path === "/api/spots") {
        const mode=url.searchParams.get("mode") ?? "mine", filters=(url.searchParams.get("filters") ?? "").split(",").filter(Boolean), latitude=Number(url.searchParams.get("latitude")), longitude=Number(url.searchParams.get("longitude")), latitudeDelta=Number(url.searchParams.get("latitudeDelta")), longitudeDelta=Number(url.searchParams.get("longitudeDelta")), radiusMiles=Math.min(Math.max(Number(url.searchParams.get("radiusMiles")) || 0, 0), 50), wantsClusters=url.searchParams.get("cluster")==="1"; const friends=await friendIds(env,user.id);
        const rows=await all(env.DB.prepare(mode==="friends" ? "SELECT * FROM spots WHERE visibility='friends'" : mode==="public" ? "SELECT * FROM spots WHERE visibility='public'" : mode==="discover" ? "SELECT * FROM spots WHERE visibility IN ('friends','public')" : "SELECT * FROM spots WHERE user_id=?").bind(...(mode==="mine"?[user.id]:[])));
        const filtered=rows.filter((spot)=>mode==="friends"?friends.has(spot.user_id):mode==="public"?spot.user_id!==user.id:mode==="discover"?(spot.visibility==="public"?spot.user_id!==user.id:friends.has(spot.user_id)):true).filter((spot)=>!filters.length||filters.includes(spot.category)).filter((spot)=>radiusMiles?isWithinMiles(spot,latitude,longitude,radiusMiles):isInVisibleMapArea(spot,latitude,longitude,latitudeDelta,longitudeDelta)).sort((a,b)=>Number.isFinite(latitude)?(a.latitude-latitude)**2+(a.longitude-longitude)**2-((b.latitude-latitude)**2+(b.longitude-longitude)**2):0);
        if (wantsClusters) {
      ...
      if (request.method === "POST" && path === "/api/spots") {
        ...
        if (!categories.has(category) || !["private","friends","public"].includes(visibility) || !Number.isFinite(Number(personalRating))) return json({error:"Choose a category, rating and visibility."},400);
        ...
        if (visibility === "friends") await Promise.all([...(await friendIds(env,user.id))].map((friendId)=>notify(env,friendId,user.id,"recommendation",spot.id,{name:spot.name})));
        return json(await shapeSpot(env,spot),201);
      }
      const spotId = path.match(/^\/api\/spots\/([^/]+)$/)?.[1];
      if (request.method==="PATCH" && spotId) { const spot=await first(env.DB.prepare("SELECT * FROM spots WHERE id=?").bind(decodeURIComponent(spotId))); if(!spot||spot.user_id!==user.id)return json({error:"Only the person who pinned this place can edit it."},404); const {visibility,category,personalRating,description}=await body(request); const nextVisibility=visibility===undefined?spot.visibility:visibility,nextCategory=category===undefined?spot.category:category,nextRating=personalRating===undefined?spot.personal_rating:Number(personalRating),nextDescription=description===undefined?spot.description:String(description).trim().slice(0,280); if(!["private","friends","public"].includes(nextVisibility)||!categories.has(nextCategory)||!Number.isFinite(nextRating)||nextRating<1||nextRating>5)return json({error:"Choose a valid category, rating and visibility."},400); await env.DB.prepare("UPDATE spots SET visibility=?,category=?,personal_rating=?,description=? WHERE id=?").bind(nextVisibility,nextCategory,nextRating,nextDescription,spot.id).run(); return json(await shapeSpot(env,{...spot,visibility:nextVisibility,category:nextCategory,personal_rating:nextRating,description:nextDescription})); }
      ...
      if(request.method==="POST"&&path==="/api/friends/requests"){const {username}=await body(request);const name=String(username??"").trim().toLowerCase().replace(/^@/,"");const recipient=await first(env.DB.prepare("SELECT * FROM users WHERE username=?").bind(name));if(!recipient)return json({error:"No Recs account has that username."},404);if(recipient.id===user.id)return json({error:"You cannot add yourself."},400);const blocked=await first(env.DB.prepare("SELECT 1 FROM user_blocks WHERE (blocker_id=? AND blocked_id=?) OR (blocker_id=? AND blocked_id=?)").bind(user.id,recipient.id,recipient.id,user.id));if(blocked)return json({error:"This account cannot receive your friend request."},403);if(recipient.friend_request_policy==="nobody")return json({error:"This account is not accepting friend requests."},403);if(recipient.friend_request_policy==="mutuals"){const mutual=await first(env.DB.prepare("SELECT 1 FROM friendships a JOIN friendships b ON ((a.requester_id=b.requester_id OR a.requester_id=b.recipient_id OR a.recipient_id=b.requester_id OR a.recipient_id=b.recipient_id)) WHERE a.status='accepted' AND b.status='accepted' AND (a.requester_id=? OR a.recipient_id=?) AND (b.requester_id=? OR b.recipient_id=?) LIMIT 1").bind(user.id,user.id,recipient.id,recipient.id));if(!mutual)return json({error:"This account only accepts requests from people with mutual friends."},403);}const existing=await first(env.DB.prepare("SELECT id FROM friendships WHERE (requester_id=? AND recipient_id=?) OR (requester_id=? AND recipient_id=?)").bind(user.id,recipient.id,recipient.id,user.id));if(existing)return json({error:"A friend request or friendship already exists."},409);const requestId=id();await env.DB.prepare("INSERT INTO friendships (id,requester_id,recipient_id,status,created_at) VALUES (?,?,?,?,?)").bind(requestId,user.id,recipient.id,"pending",now()).run();await notify(env,recipient.id,user.id,"friend_request",requestId);return json({id:requestId},201);}
      const acceptId=path.match(/^\/api\/friends\/requests\/([^/]+)\/accept$/)?.[1];
      if(request.method==="POST"&&acceptId){const friendship=await first(env.DB.prepare("SELECT * FROM friendships WHERE id=?").bind(acceptId));if(!friendship)return json({error:"Friend request not found."},404);if(friendship.recipient_id!==user.id)return json({error:"Only the invited person can accept this request."},403);if(friendship.status==="pending"){await env.DB.prepare("UPDATE friendships SET status='accepted' WHERE id=?").bind(acceptId).run();await notify(env,friendship.requester_id,user.id,"friend_accepted",acceptId);}return json({id:acceptId,status:"accepted"});}
      const rejectId=path.match(/^\/api\/friends\/requests\/([^/]+)$/)?.[1];
      if(request.method==="DELETE"&&rejectId){const friendship=await first(env.DB.prepare("SELECT * FROM friendships WHERE id=?").bind(rejectId));if(!friendship||friendship.status!=="pending")return json({error:"Friend request not found."},404);if(friendship.recipient_id!==user.id)return json({error:"Only the invited person can reject this request."},403);await env.DB.prepare("DELETE FROM friendships WHERE id=?").bind(rejectId).run();return empty();}
      const removeFriendId=path.match(/^\/api\/friends\/([^/]+)$/)?.[1];
      if(request.method==="DELETE"&&removeFriendId){const friendId=decodeURIComponent(removeFriendId),friendship=await first(env.DB.prepare("SELECT * FROM friendships WHERE status='accepted' AND ((requester_id=? AND recipient_id=?) OR (requester_id=? AND recipient_id=?))").bind(user.id,friendId,friendId,user.id));if(!friendship)return json({error:"Friendship not found."},404);await env.DB.batch([
        env.DB.prepare("DELETE FROM saved_places WHERE user_id=? AND spot_id IN (SELECT id FROM spots WHERE user_id=? AND visibility='friends')").bind(user.id,friendId),
        env.DB.prepare("DELETE FROM saved_places WHERE user_id=? AND spot_id IN (SELECT id FROM spots WHERE user_id=? AND visibility='friends')").bind(friendId,user.id),
        env.DB.prepare("DELETE FROM plan_invites WHERE user_id=? AND plan_id IN (SELECT id FROM plans WHERE host_id=?)").bind(friendId,user.id),
        env.DB.prepare("DELETE FROM plan_invites WHERE user_id=? AND plan_id IN (SELECT id FROM plans WHERE host_id=?)").bind(user.id,friendId),
        env.DB.prepare("DELETE FROM plans WHERE host_id IN (?,?) AND NOT EXISTS (SELECT 1 FROM plan_invites i WHERE i.plan_id=plans.id)").bind(user.id,friendId),
        env.DB.prepare("DELETE FROM friendships WHERE id=?").bind(friendship.id),
      ]);return empty();}
      ...
      if(request.method==="GET"&&path==="/api/me/privacy")return json({friendRequestPolicy:user.friend_request_policy??"everyone"});
      if(request.method==="GET"&&path==="/api/activity"){const rows=await all(env.DB.prepare("SELECT a.*,u.name AS actor_name,u.username AS actor_username FROM activity a LEFT JOIN users u ON u.id=a.actor_id WHERE a.recipient_id=? ORDER BY a.created_at DESC LIMIT 30").bind(user.id));return json(rows.map(row=>({...row,payload:JSON.parse(row.payload||"{}"),actor:row.actor_id?{id:row.actor_id,name:row.actor_name,username:row.actor_username}:null})));}
      if(request.method==="PATCH"&&path==="/api/me/privacy"){const {friendRequestPolicy}=await body(request);if(!["everyone","mutuals","nobody"].includes(friendRequestPolicy))return json({error:"Choose a valid friend request setting."},400);await env.DB.prepare("UPDATE users SET friend_request_policy=? WHERE id=?").bind(friendRequestPolicy,user.id).run();return json({friendRequestPolicy});}
      if(request.method==="GET"&&path==="/api/me/blocks"){const rows=await all(env.DB.prepare("SELECT u.* FROM user_blocks b JOIN users u ON u.id=b.blocked_id WHERE b.blocker_id=? ORDER BY b.created_at DESC").bind(user.id));return json(rows.map(publicUser));}
      if(request.method==="POST"&&path==="/api/reports"){const {targetType,targetId,reason}=await body(request);if(!["spot","user"].includes(targetType)||!String(targetId??"").trim()||!String(reason??"").trim())return json({error:"Choose what you are reporting and tell us why."},400);if(targetType==="spot"){const spot=await first(env.DB.prepare("SELECT * FROM spots WHERE id=?").bind(String(targetId)));if(!spot||!await canView(env,spot,user.id))return json({error:"That recommendation is no longer available."},404);}else{const target=await first(env.DB.prepare("SELECT id FROM users WHERE id=?").bind(String(targetId)));if(!target||target.id===user.id)return json({error:"Choose another account to report."},400);}await env.DB.prepare("INSERT OR IGNORE INTO reports (id,reporter_id,target_type,target_id,reason,created_at) VALUES (?,?,?,?,?,?)").bind(id(),user.id,targetType,String(targetId),String(reason).trim().slice(0,280),now()).run();return json({ok:true},201);}
      if(request.method==="POST"&&path==="/api/me/blocks"){const {username}=await body(request),name=String(username??"").trim().toLowerCase().replace(/^@/,"");const blocked=await first(env.DB.prepare("SELECT * FROM users WHERE username=?").bind(name));if(!blocked||blocked.id===user.id)return json({error:"Choose another Recs account to block."},400);await env.DB.batch([env.DB.prepare("INSERT OR IGNORE INTO user_blocks (blocker_id,blocked_id,created_at) VALUES (?,?,?)").bind(user.id,blocked.id,now()),env.DB.prepare("DELETE FROM friendships WHERE (requester_id=? AND recipient_id=?) OR (requester_id=? AND recipient_id=?)").bind(user.id,blocked.id,blocked.id,user.id)]);return json(publicUser(blocked),201);}
      const unblockId=path.match(/^\/api\/me\/blocks\/([^/]+)$/)?.[1];if(request.method==="DELETE"&&unblockId){await env.DB.prepare("DELETE FROM user_blocks WHERE blocker_id=? AND blocked_id=?").bind(user.id,decodeURIComponent(unblockId)).run();return empty();}
      if(request.method==="PATCH"&&path==="/api/me"){const {photoUri,name,username,email}=await body(request);if(photoUri)return json({error:"Photo uploads are not enabled in this hosted preview."},400);const nextName=name===undefined?user.name:String(name).trim();const nextUsername=username===undefined?user.username:String(username).trim().toLowerCase().replace(/^@/,"");const nextEmail=email===undefined?user.email:String(email).trim().toLowerCase();if(!nextName||!nextUsername||!nextEmail.includes("@"))return json({error:"Enter a name, username and valid email address."},400);const clash=await first(env.DB.prepare("SELECT id FROM users WHERE (email=? OR username=?) AND id<>?").bind(nextEmail,nextUsername,user.id));if(clash)return json({error:"That email or username is already in use."},409);await env.DB.prepare("UPDATE users SET name=?,username=?,email=?,photo_uri=NULL WHERE id=?").bind(nextName,nextUsername,nextEmail,user.id).run();return json(publicUser({...user,name:nextName,username:nextUsername,email:nextEmail,photo_uri:null}));}
      ...
      if(request.method==="POST"&&path==="/api/plans"){const {spotId,scheduledAt,inviteeIds=[]}=await body(request),spot=await first(env.DB.prepare("SELECT * FROM spots WHERE id=?").bind(spotId));if(!spot||!await canView(env,spot,user.id)||new Date(scheduledAt)<=new Date())return json({error:"Choose an available place and future date."},400);const allowed=await friendIds(env,user.id),recipients=[...new Set(inviteeIds)].filter(i=>allowed.has(i));if(!recipients.length)return json({error:"Choose at least one friend."},400);const planId=id();await env.DB.batch([env.DB.prepare("INSERT INTO plans (id,spot_id,host_id,scheduled_at,created_at) VALUES (?,?,?,?,?)").bind(planId,spot.id,user.id,new Date(scheduledAt).toISOString(),now()),...recipients.map(i=>env.DB.prepare("INSERT INTO plan_invites (plan_id,user_id,status) VALUES (?,?,?)").bind(planId,i,"pending"))]);await Promise.all(recipients.map(recipientId=>notify(env,recipientId,user.id,"plan_invite",planId,{spotName:spot.name,scheduledAt})));return json({id:planId},201);}
      const rsvp=path.match(/^\/api\/plans\/([^/]+)\/respond$/)?.[1];
      if(request.method==="POST"&&rsvp){const {status}=await body(request);if(!["accepted","maybe","declined"].includes(status))return json({error:"Invalid response."},400);const plan=await first(env.DB.prepare("SELECT * FROM plans WHERE id=?").bind(rsvp));const result=await env.DB.prepare("UPDATE plan_invites SET status=? WHERE plan_id=? AND user_id=?").bind(status,rsvp,user.id).run();if(result.meta.changes&&plan)await notify(env,plan.host_id,user.id,"plan_rsvp",rsvp,{status});return result.meta.changes?json({id:rsvp,status}):json({error:"Plan invitation not found."},404);}
