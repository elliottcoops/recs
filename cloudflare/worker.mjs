  ...
  async fetch(request, env) {
    ...
    try {
      ...
      if (request.method === "GET" && path === "/api/spots") {
        ...
        const rows=await all(env.DB.prepare(mode==="friends" ? "SELECT * FROM spots WHERE visibility='friends'" : mode==="public" ? "SELECT * FROM spots WHERE visibility='public'" : "SELECT * FROM spots WHERE user_id=?").bind(...(mode==="mine"?[user.id]:[])));
        const filtered=rows.filter((spot)=>mode==="friends"?friends.has(spot.user_id):mode==="public"?spot.user_id!==user.id:true).filter((spot)=>!filters.length||filters.includes(spot.category)).filter((spot)=>radiusMiles?isWithinMiles(spot,latitude,longitude,radiusMiles):isInVisibleMapArea(spot,latitude,longitude,latitudeDelta,longitudeDelta)).sort((a,b)=>Number.isFinite(latitude)?(a.latitude-latitude)**2+(a.longitude-longitude)**2-((b.latitude-latitude)**2+(b.longitude-longitude)**2):0);
        if (wantsClusters) {
